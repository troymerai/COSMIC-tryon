...
class AlignedDataset(BaseDataset):
    ...
    def get_file_name(self):
        im = sorted(os.listdir(self.dir_I))[0]
        self.im_name.append(os.path.join(self.dir_I, im))

        c_n = sorted(os.listdir(self.dir_C))[0]
        self.c_name.append(os.path.join(self.dir_C, c_n))

        e_n = sorted(os.listdir(self.dir_E))[0]
        self.e_name.append(os.path.join(self.dir_E, e_n))
    ...
    def __getitem__(self, index):
        ...
        input_dict = {'image': I_tensor, 'clothes': C_tensor,
                      'edge': E_tensor, 'p_name': self.im_name[index].split('/')[-1]}
        return input_dict
